...
class ResearchManager:
    ...
    async def run(self, query: str) -> str:
        with custom_span("Research trace"):
            self.printer.update_item(
                "trace_id",
                f"View trace: https://platform.openai.com/traces/trace?trace_id={get_current_trace().trace_id}",
                is_done=True,
                hide_checkmark=True,
            )
    ...
    async def _perform_searches(self, search_plan: WebSearchPlan) -> list[str]:
        with custom_span("Search the web"):
            ...
            # Uncomment the following lines if you want to use a synchronous loop instead of asyncio.as_completed
            # The tracing works fine with the synchronous loop
            # for i, item in enumerate(search_plan.searches, 1):
            #     result = await self._search(item)
            #     if result is not None:
            #         results.append(result)
            #     self.printer.update_item(
            #         "searching", f"Searching... {i}/{len(search_plan.searches)} completed"
            #     )
