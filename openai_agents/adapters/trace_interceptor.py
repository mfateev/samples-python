...
from agents import get_current_span, custom_span, trace, CustomSpanData
...
def context_from_header(
        span_name: str, input: _InputWithHeaders, payload_converter: temporalio.converter.PayloadConverter
):
    ...
    if span_info is None:
        ...
        workflow_type = activity.info().workflow_type if activity.in_activity() else workflow.info().workflow_type
        data = {"activityId": activity.info().activity_id} if activity.in_activity() else None
        if get_trace_provider().get_current_trace() is None:
            metadata = {
                "temporal:workflowId": activity.info().workflow_id if activity.in_activity() else workflow.info().workflow_id,
                "temporal:runId": activity.info().workflow_run_id if activity.in_activity() else workflow.info().run_id,
                "temporal:workflowType": workflow_type,
            }
            with trace(span_info["traceWorkflowName"], trace_id=span_info["traceId"], metadata=metadata):
...
class _ContextPropagationClientOutboundInterceptor(
    ...
    async def start_workflow(
        ...
        if get_trace_provider().get_current_trace() is None:
            with trace(span_name + ":" + input.workflow, metadata=metadata, group_id=input.id):
...
class _ContextPropagationActivityInboundInterceptor(
    ...
    async def execute_activity(
        ...
        with context_from_header("temporal:executeActivity", input, temporalio.activity.payload_converter()):
...
class _ContextPropagationWorkflowInboundInterceptor(
    ...
    async def execute_workflow(
        ...
        with context_from_header("temporal:executeWorkflow", input, temporalio.workflow.payload_converter()):
            return await self.next.execute_workflow(input)

    async def handle_signal(self, input: temporalio.worker.HandleSignalInput) -> None:
        with context_from_header("temporal:handleSignal", input, temporalio.workflow.payload_converter()):
            return await self.next.handle_signal(input)

    async def handle_query(self, input: temporalio.worker.HandleQueryInput) -> Any:
        with context_from_header("temporal:handleQuery", input, temporalio.workflow.payload_converter()):
            return await self.next.handle_query(input)

    def handle_update_validator(
            self, input: temporalio.worker.HandleUpdateInput
    ) -> None:
        with context_from_header("temporal:handleUpdateValidator", input, temporalio.workflow.payload_converter()):
            self.next.handle_update_validator(input)

    async def handle_update_handler(
            self, input: temporalio.worker.HandleUpdateInput
    ) -> Any:
        with context_from_header("temporal:handleUpdateHandler", input, temporalio.workflow.payload_converter()):
